...
*Part 1 of 3 · [Part 2: CRUX-Land run report →](/drafts/crux-land) · [Part 3: the synthesis →](/drafts/path-to-agi)*
...
┌──────────────────────────────────────┐
│  CRUX-X Methodology                  │   artifact: ~39 KB markdown spec
│  (methodology.md)                    │   defines [SLOT]s the Designer fills:
│    §1  Hypothesis            [SLOT]  │
│    §3  Inputs / pre-staged   [SLOT]  │
│    §5  Comms + kill switch   [SLOT]  │
│    §7  Budget / constraints  [SLOT]  │
│    §8  Writeup template      [SLOT]  │
...
└──────────────────┬───────────────────┘
            ...
            reads + fills
                   │
                   ▼
            « Designer »      agent — an LLM session, one shot per task
                   │
                drafts
                   │
                   ▼
┌──────────────────────────────────────┐
│  CRUX-Windows Protocol               │   artifact: task-specific instance
│  (experiments/windows/protocol.md)   │   each [SLOT] resolved concretely:
│    §1 → "Publish a Windows app       │
│          to the Microsoft Store"     │
│    §3 → Partner Center dev account,  │
│          Gmail, GitHub PAT, signing  │
│          cert, controller VM         │
│    §5 → Slack #crux-windows, kill    │
│          switch via gateway-stop     │
│    §7 → $500 Anthropic API cap,      │
│          14-day wall-clock cap       │
└──────────────────┬───────────────────┘
                   │
         reviews + approves
                   │
                   ▼
            « Operator »      agent — the human (me)
                   │
              kicks off
                   │
                   ▼
...
│  (runs/2026-04-13/manifest.json)     │
│    run-id, t=0 timestamp, model id,  │
│    budget caps snapshot, vault refs  │
└──────────────────┬───────────────────┘
                   │
                 binds
                   │
                   ▼
             « Agent »        agent — autonomous LLM, long-running session
                   │
         executes against
        (protocol + manifest)
                   │
        ...
        ════ The Run ════           execution — wall-clock window
                                    (CRUX-Windows: 77h 48m, 3 net inputs)
                   │
            interacts with
                   │
                   ▼
         Counterparty(ies)          external real-world gatekeeper
         (CRUX-Windows: Microsoft
          Partner Center reviewers)
```

The Manifest is a separate artifact from the Protocol on purpose. The Protocol is the task spec — the same across runs of the same experiment. The Manifest snapshots runtime state: which model was used, what the budget caps were at t=0, which credential vault, which controller VM, the t=0 timestamp itself. To re-run CRUX-Windows six months from now against a newer model, I draft a fresh Manifest against the same Protocol. Reproducibility lives in the Manifest.
...
The constraint stack is tight on purpose. A $1,500 parcel cap forces the agent into the actual cheap-rural-parcel market — county tax-deed sales, regional title companies, state-bar real estate attorneys, small recorder offices. That's the segment where the gap between benchmark performance and real-world completion gets ugly. It's also the segment where, if you were actually buying off-grid land, you'd actually shop.

CRUX-Land kicked off 2026-04-29 and terminated as primary-failed on 2026-05-04 — six days, twelve evaluated channels, twelve dead candidates. Net real-money loss after the deposit refund: $30.
