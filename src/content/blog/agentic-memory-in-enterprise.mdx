...
1. **How the filesystem memory is actually implemented.** The experiment ran against a local directory tree on a single machine. Production enterprise memory lives in a distributed store — S3, GCS, a git service, a document CMS, possibly a hybrid with a search index bolted on. That introduces concerns the local experiment didn't touch: read latency, cache invalidation, consistency (eventual vs. strong), access-control integration with the corporate identity provider, and observability (what does `grep` mean when it spans a remote store?). Arm B's cost advantage could erode once network latency enters the picture — or it could hold, if the retrieval primitives cache well. Worth testing. A promising project in this direction is https://archil.com/ which already has primitives for mounting several clients on a shared filesystem (https://docs.archil.com/concepts/shared-disks) 

2. **The write path.** Reads only in this experiment. A real team memory layer has to handle writes: who can edit team docs, how to reconcile concurrent edits from multiple team members or agents, what "append to an existing runbook" means when two agents do it simultaneously. CRDT, operational transform, and PR-based review workflows all apply; which one fits memory specifically — content humans author but agents read and reference — is an open design question. Most of the hard security concerns move here once reads work, since write authority is strictly narrower than read authority in every real org.

3. **Subagent-level access attenuation.** I tested team-tier access (whole identities scoped to teams). Finer-grained scoping — a subagent handling one subtask getting only the memory slice its task needs, not the full reach of its parent — is a follow-up problem. It becomes load-bearing once you start spawning subagents with narrower reach than the parent. I didn't test the obvious design here (capability tokens that attenuate as they flow down a chain).

4. **Variance.** n=1 per cell gives directional signal, not a publishable number. The 4.7× token ratio could be 3× or 7× under repeated runs; the direction is solid but the magnitude is rough.

5. **Temporal dynamics.** The memory I tested is a snapshot. Real team memory ages — runbooks go stale, post-mortems lose relevance, eval thresholds move. How an agent should weight recent vs. older memory, and how the store should garbage-collect or archive stale docs, is the domain of papers like [REMem](https://arxiv.org/abs/2602.13530), [HippoRAG 2](https://arxiv.org/abs/2502.14802), and [Zep](https://arxiv.org/abs/2501.13956). None of those papers address permissioning; nothing I built here addresses temporality. Bridging the two is its own experiment.

6. **Adversarial retrieval.** What happens when an authorized-for-some identity grep-mines for sensitive strings scoped to a different team, or uses the agent as an oracle to test whether a specific fact exists in memory they shouldn't see? Arm A's "path does not exist" error partially defends against existence disclosure. Arm B's filesystem leaks existence by default — `ls` still lists unauthorized directory names, even though the OS denies access to their contents. I didn't probe either arm adversarially in this experiment.

7. **Model dependence.** Arm B's advantage leans on the model guessing filenames well; a weaker model might not. The usual fix — co-locating metadata with each file (frontmatter summaries, a curated team index) so the agent doesn't have to infer paths — isn't likely to matter until the corpus outgrows what names alone can carry.
