...
Run 1 evidence: the Bid4Assets bidder account doesn't fit on either side of an "agent-vs-operator" binary. Six axes per account, three different actors across them, all on the same account. Part 2's safety-policy section has the full breakdown for B4A.

Fix: methodology §3 adds `[DECISION] per-account ownership table` as a pre-kickoff artifact. For each counterparty account the experiment uses, the Designer maps ownership across the six axes at design time, not mid-run. Mapping at design time prevents the methodology from systematically underestimating what agents can do at KYC counterparties.
...
CRUX-Land hit five surfaces requiring the operator's legal identity in the first three days (Part 2's safety-policy section enumerates them). None are model-capability gaps. They're identity-and-authority gaps. The agent doesn't have a real-world legal identity; the operator does. KYC binds the transaction to the operator's identity; no protocol or methodology change can lift that ceiling.
